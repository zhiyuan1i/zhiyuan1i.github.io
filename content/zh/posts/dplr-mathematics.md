...
DPLR 相比标准 Delta Rule 的优势：显式控制对角衰减（dim-wise forgetting）和低秩更新，表达力更强，但在 chunk 形势下显著的引入了额外的计算复杂度，需要更多的 HBM 空间来存储中间变量。
...
与标准 DPLR 相比，RWKV-7 的关键设计包括：
- **动态的衰减与学习率**：$w_t, a_t, b_t$ 都是 data-dependent 的，允许模型根据上下文动态决定遗忘和更新的强度。
- **表达能力的飞跃**：由于引入了显式的状态演化，RWKV-7 能够识别所有正则语言（regular languages），其理论表达能力超越了 TC0 的 Transformer，达到了 NC1。
...
| $\mathbf{K}, \mathbf{V}, \mathbf{A}^{\text{lr}}, \mathbf{B}^{\text{lr}}$ | $\mathbb{R}^{C \times K}$ / $\mathbb{R}^{C \times V}$ | chunk-level 矩阵（低秩向量 $a, b$ 的矩阵形式），第 $i$ 行为对应向量 |
...
设 $\mathbf{P}_i = \mathbf{D}_i + \mathbf{b}_i \mathbf{a}_i^T$，其中 $\mathbf{D}_i$ 是对角矩阵。则累积乘积 $\mathbf{P}_{1:t} = \prod_{i=1}^t \mathbf{P}_i$ 可以表示为：

$$\mathbf{P}_{1:t} = \boldsymbol{\Gamma}_1^t + \sum_{i=1}^t (\boldsymbol{\Gamma}_1^{i-1} \mathbf{b}_i) \cdot (\mathbf{a}_i^T \boldsymbol{\Gamma}_{i+1}^t)$$
...
- $\mathbf{A}^{\text{gate}} = \mathbf{A}^{\text{lr}} \odot \exp(\mathbf{G}^{\text{cum}})$（门控后的低秩向量矩阵）
...
其中 $\tilde{\mathbf{u}}_i^T = \mathbf{u}_i^T + \mathbf{w}_i^T \mathbf{S}$ 是包含历史状态贡献的修正向量（注意这里的转置）。

代入 WY 表示的矩阵形式 $\tilde{\mathbf{U}} = \mathbf{U} + \mathbf{W} \mathbf{S}$：
...
3. **分块计算**：当 $K$ 较大时，需要将 key/value 维度分块以适配 GPU Shared Memory

4. **精度控制**：类似 KDA，中间计算使用 float32，存储使用 bf16/fp16
