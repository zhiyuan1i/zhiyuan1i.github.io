...
Advantages of DPLR over standard Delta Rule: explicit control of diagonal decay (dim-wise forgetting) and low-rank updates, providing stronger expressiveness. However, in the chunk-wise form, it significantly introduces additional computational complexity and requires more HBM space to store intermediate variables.
...
Key design features of RWKV-7 compared to standard architectures:
...
- **Leap in Expressiveness**: By introducing explicit state evolution, RWKV-7 can recognize all regular languages. Its theoretical expressiveness surpasses TC0 (Transformers) and reaches NC1.
...
| $\mathbf{K}, \mathbf{V}, \mathbf{A}^{\text{lr}}, \mathbf{B}^{\text{lr}}$ | $\mathbb{R}^{C \times K}$ / $\mathbb{R}^{C \times V}$ | Chunk-level matrices (matrix forms of low-rank vectors $a, b$), row $i$ is the corresponding vector |
...
Let $\mathbf{P}_i = \mathbf{D}_i + \mathbf{a}_i^T \mathbf{b}_i$, where $\mathbf{D}_i$ is a diagonal matrix. Then the cumulative product $\mathbf{P}_{1:t} = \prod_{i=1}^t \mathbf{P}_i$ can be expressed as:

$$\mathbf{P}_{1:t} = \boldsymbol{\Gamma}_1^t + \sum_{i=1}^t (\boldsymbol{\Gamma}_1^{i-1} \mathbf{a}_i^T) \cdot (\mathbf{b}_i \boldsymbol{\Gamma}_{i+1}^t)$$

**Significance**: This lemma guarantees that the DPLR structure is closed under matrix multiplication, forming the foundation for the existence of the WY representation.
...
- $\mathbf{A}^{\text{gate}} = \mathbf{A}^{\text{lr}} \odot \exp(\mathbf{G}^{\text{cum}})$ (gated low-rank vector matrix)
...
This is completely symmetric to the WY representation in KDA!
...
$$
\begin{aligned}
\mathbf{S}' &= \text{diag}(\exp(\mathbf{g}_{\text{last}})) \mathbf{S} + \sum_{i=1}^C \exp(\mathbf{g}_{\text{last}} - \mathbf{g}_i) (\mathbf{k}_i^T \mathbf{v}_i + \mathbf{b}_i \tilde{\mathbf{u}}_i^T) \\
&= \text{diag}(\exp(\mathbf{g}_{\text{last}})) \mathbf{S} + \mathbf{K}_{\text{decayed}}^T \mathbf{V} + \mathbf{B}_{\text{decayed}}^T \tilde{\mathbf{U}}
\end{aligned}
$$

Where $\tilde{\mathbf{u}}_i^T = \mathbf{u}_i^T + \mathbf{w}_i^T \mathbf{S}$ is the corrected vector including historical state contributions (note the transpose here).

Substituting the WY representation's matrix form $\tilde{\mathbf{U}} = \mathbf{U} + \mathbf{W} \mathbf{S}$:
...
3. **Block-wise Computation**: When $K$ is large, key/value dimensions need to be blocked to fit GPU Shared Memory

4. **Precision Control**: Similar to KDA, intermediate computations use float32, while storage uses bf16/fp16
